################################################################################
# Name: adh_numerics_1spec_tophat.py
#
# Purpose: Numerics for 1 species memory-advection-diffusion model, with periodic
#          boundary conditions, and top hat kernel
#
# Usage: python adh_numerics_1spec_tophat.py 100000 100000 200 100 0.1 2 1 1 1 8 adh_ic_gauss_sigma0.05.inp ..\..\..\..\OneDrive\ADH\adh_numerics_1spec_tophat_10000_100000_200_100_0.1_2_1_1_1_8.out
################################################################################
...
for time in range(max_time):
    # Smoothed arrays
    for space in range(space_granularity):
        minx = int(space-delta*space_granularity/box_width)
        maxx = int(space+delta*space_granularity/box_width)+1
    ...
    for space in range(space_granularity):
        ...
        new_u_array[space] = old_u_array[space] + (delta_t/(delta_x**2))*(
                                     ...
                                     (((gamma*(smooth_u_array[(space+2) % space_granularity] - smooth_u_array[space])+(env[(space+2) % space_granularity] - env[space]))*old_u_array[(space+1) % space_granularity]) - 
                                      ((gamma*(smooth_u_array[space] - smooth_u_array[(space-2) % space_granularity])+(env[space] - env[(space-2) % space_granularity]))*old_u_array[(space-1) % space_granularity]))/4) 
